...
import { Plus, BarChart3, Clock, Trash2, Share2, ExternalLink, Loader2 } from 'lucide-react';
import API from '../api';

const PollCard = ({ poll, onDelete }) => {
  // NEW: Logic to calculate the sum of all votes from all questions
  const totalVotesCount = poll.questions?.reduce((acc, q) => {
    return acc + (q.options?.reduce((sum, opt) => sum + (opt.votes || 0), 0) || 0);
  }, 0) || 0;
    ...
    <div className="glass p-8 rounded-[32px] hover:border-blue-500/30 transition-all group relative border border-white/10">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={copyToClipboard} className="p-2 rounded-lg hover:bg-blue-500/10 text-blue-500 transition-colors" title="Share">
            <Share2 size={18}/>
          </button>
          <button onClick={() => onDelete(poll._id)} className="p-2 rounded-lg hover:bg-red-500/10 text-red-500 transition-colors" title="Delete">
            <Trash2 size={18}/>
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-6 text-sm font-bold opacity-40 uppercase tracking-widest">
        ...
        <div className="flex items-center gap-2"><Clock size={14}/> Live</div>
      </div>
    </div>
  );
};
    ...
    <div className="pt-32 px-6 max-w-7xl mx-auto min-h-screen">
      <header className="flex justify-between items-end mb-12">
        <div>
          ...
          <p className="opacity-50 font-medium mt-2">Manage your real-time feedback loops</p>
        </div>
        <Link 
          to="/create-poll" 
      ...
      {polls.length === 0 ? (
        <div className="glass p-20 rounded-[40px] text-center border-dashed border-2 border-white/10">
          <p className="text-xl font-bold opacity-30 uppercase tracking-widest">No pulses found</p>
          <Link to="/create-poll" className="text-blue-500 font-bold mt-4 inline-block hover:underline">
